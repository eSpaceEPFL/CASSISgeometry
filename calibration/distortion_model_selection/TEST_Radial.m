...
cost_fun_pix = @(params, PREDICTOR, RESPONSE)   mean(sqrt(sum( ((RESPONSE - distort_radial(params, PREDICTOR) ).^2).*(height+width).^2, 2)));

% find optimal params
param = fit_Radial(PREDICTOR, RESPONSE);

% perform 10-fold test
fun = @(XTRAIN, YTRAIN, XTEST, YTEST) cost_fun_pix(fit_Radial(XTRAIN, YTRAIN), XTEST, YTEST);
mrse = crossval(fun, PREDICTOR, RESPONSE, 'Leaveout', 1);

% plot distortion field 
[i, j, x, y] = simulate_distortion_field(@distort_radial, param, image_size);
