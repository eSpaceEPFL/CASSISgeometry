...
for nseq = 1:nb_seq
    ...
    for nExp = 1+prm.skip_first_exp:nb_exp 
        ...
        [exp, mask] = seq.getExp(nExp, [1:seq.getSubExpNb()], false, prm.virtualImage_on, prm.adjust_subExp_on);
        ...
        imwrite(uint16(exp*2^16), [set.raw_exposures '/' fname_exp{nexp_ttl}]);
        imwrite((mask > 0), [set.raw_exposures '/' fname_mask{nexp_ttl}]);
        nexp_ttl = nexp_ttl + 1;
